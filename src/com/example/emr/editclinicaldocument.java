...
public class editclinicaldocument extends Activity {
	int count = 2;
	...
	protected void editData() {
		...
		try {
			...
			HttpPost httppost = new HttpPost("http://emrophthalmology.comli.com/editEyediseases.php");
	...
	public void getEyeDisease() {
		...
		try {
			...
			HttpPost httppost = new HttpPost(
					"http://emrophthalmology.comli.com/getEyeDisease.php");
			httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs));
			HttpResponse response = httpclient.execute(httppost);
			HttpEntity entity = response.getEntity();
			is = entity.getContent();
		...
		try {
			...
			if (success == 1) {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					String[] ed = jsoneyedisease.getString("eyedisease").split("; ");
					for(int j = 0; j < ed.length; j++){
						if(j>0)
							addEyedisease();
						eyediseases.get(j).setText(ed[j]);
					}
					etdescription.setText(jsoneyedisease.getString("description"));
				}
			} else {
